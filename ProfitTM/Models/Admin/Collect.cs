            ...
            using (ProfitAdmEntities context = new ProfitAdmEntities(entity.ToString()))
            {
                using (DbContextTransaction tran = context.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        saFacturaVenta fact = context.saFacturaVenta.FirstOrDefault(f => f.doc_num == cob.campo1);
                        saDocumentoVenta doc_v = context.saDocumentoVenta.FirstOrDefault(d => d.co_tipo_doc == "FACT" && d.nro_doc == cob.campo1);
                        ...
                        if (rengs.Count > 0)
                        {
                            ...
                            context.Entry(fact).Property(f => f.saldo).IsModified = true;
                            // context.Entry(fact).State = EntityState.Modified;

                            // ACTUALIZAR DOCUMENTO
                            doc_v.saldo -= total_fact;
                            context.Entry(doc_v).Property(d => d.saldo).IsModified = true;
